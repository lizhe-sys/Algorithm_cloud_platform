...
$.ajax({
	type: "get",
	url: urls.server + '/file/getLogFileList',
	async: true,
	data: {
	},
	success: function(res) {
		if (res.success) {
			for (let file of res.data) {
				...
				// } else {
				$("#fileList ul").append(
					`<li class="fl-li" id="${file.id}"><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}" ondblclick="downloadLog( this.id )" id="${file.fileName}"><label>${file.fileName}</label><span class="tooltiptext">${file.fileName}<span style="color: red"> - 双击下载</span></span></li>`
				);
				// }
			}
		}

			...
			for (let file of res.data) {
				if(file.fileName.search('.mp3')==file.fileName.length-4){
					if (file.fileName.length > 10) {
						$("#fileList ul").append(
							`<li class="fl-li" ><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}" onclick="player(this.name,this.id, 'mp3' )"  name = "${file.fileName}"  ondblclick="downloadFile( this.id )" id="${file.id}"><label>${file.fileName.substring(0, 10)}…</label><span class="tooltiptext">${file.fileName}<span style="color: red">- 单击播放 - 双击下载 </span></span></li>`
						);
					} else {
						this.files = file;
						console.log(this.files,file,'asdf', file.fileName  );
						$("#fileList ul").append(
							`<li class="fl-li"   id="${file.id}"><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}" onclick="player( this.name,this.id, 'mp3')" ondblclick="downloadFile( this.id )" name = "${file.fileName}"  id="${file.id}"><label>${file.fileName}</label><span class="tooltiptext">${file.fileName}<span style="color: red">- 单击播放 - 双击下载 </span></span></li>`
						);
					}
				}else if(file.fileName.search('.mp4') == file.fileName.length-4){
					if (file.fileName.length > 10) {
						$("#fileList ul").append(
							`<li class="fl-li" ><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}" onclick="player( this.name,this.id, 'mp4')"  name = "${file.fileName}"  ondblclick="downloadFile( this.id )" id="${file.id}"><label>${file.fileName.substring(0, 10)}…</label><span class="tooltiptext">${file.fileName}<span style="color: red">- 单击播放 - 双击下载</span></span></li>`
						);
					} else {
						$("#fileList ul").append(
							`<li class="fl-li" id="${file.id}"><i class="fileId">${file.id}</i><img src="${getFileLogo(file.suffix)}" onclick="player( this.name,this.id, 'mp4')"  name = "${file.fileName}"  ondblclick="downloadFile( this.id )" id="${file.id}"><label>${file.fileName}</label><span class="tooltiptext">${file.fileName}<span style="color: red">- 单击播放 - 双击下载 </span></span></li>`
						);
					}
